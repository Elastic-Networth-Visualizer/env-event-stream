import { Event, EventHandler, TopicOptions, SubscriptionOptions } from "./types.ts";
...
export class Topic {
  ...
  subscribe<T = unknown>(
    ...
    const subscriptionId = options.name || uuid();
    ...
    if (options.receiveHistoricalEvents) {
      this.eventStore.getEvents(this.name, {
        eventTypes: options.eventTypes
      }).then(events => {
        for (const event of events) {
          subscription.deliver(event).catch(error => {
            console.error(`Error delivering historical event ${event.id}:`, error);
          });
        }
      });
    }
  ...
  async publish<T = unknown>(event: Event<T>): Promise<number> {
    ...
    if (this.options.schemaRegistry) {
      const isValid = await this.options.schemaRegistry.validate(event as Event);
      if (!isValid) {
    ...
    for (const subscription of this.subscriptions.values()) {
      ...
      deliveryPromises.push(
        subscription.deliver(event as Event).catch(error => {
          console.error(`Error delivering event ${event.id} to subscription ${subscription.getId()}:`, error);
        })
      );
    }
